/**
 * One-time development bootstrap: create the FIRST ADMIN on the new Neon database.
 *
 * Usage:
 *   INITIAL_ADMIN_PASSWORD='...' npm run bootstrap:admin
 *
...
async function main() {
  ...
  if (strengthErrors.length > 0) {
    console.error(
      `INITIAL_ADMIN_PASSWORD is not strong enough. It must include ${strengthErrors.join(", ")}.`
    )
    process.exit(1)
  }
